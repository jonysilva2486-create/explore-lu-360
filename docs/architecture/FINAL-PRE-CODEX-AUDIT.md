...
1. **P-01:** Resolve Saved Trails vs actual MVP save scope.
2. **D-01:** Freeze production typography/fallbacks and multilingual/performance validation criteria.
3. **D-02:** Define the implementation token source of truth.
4. **G-02:** Reconcile the R3 experience-priority wording with R4.3 trail-distribution wording.
5. **G-03:** Lock the Google Maps key policy before integration: dedicated key(s), application restriction(s), API restriction(s), no secrets in repository/chat.
6. **I-01:** Decide the environment model for the current US Auth0 development tenant and EU production identity tenancy.
7. **O-02:** Decide/document EU-first production runtime/database region, with Frankfurt considered for Render.
8. **S-01:** Manually verify `main` branch protection.
9. **S-02:** Establish the minimum CI workflow and prove it runs successfully once foundation code exists.
